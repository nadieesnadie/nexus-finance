    ...
    <div className="flex h-screen bg-[#0a0518] text-white font-sans antialiased overflow-hidden selection:bg-violet-500/30">
      ...
      >
        ...
        <nav className="flex flex-col gap-4 w-full px-2">
          <SidebarItem icon={<LayoutDashboard size={24} />} label={t.terminal} active expanded={isSidebarExpanded} />
        </nav>
      </motion.aside>

      {/* MAIN VIEW */}
      <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10">
        <div className="p-8 lg:p-12 max-w-[1800px] mx-auto pb-32">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-16">
            ...
            <div className="xl:col-span-8 flex flex-col gap-12">
              ...
              <div className="bg-black/30 border border-white/10 rounded-[2.5rem] p-10 min-h-[650px] flex flex-col relative overflow-visible shadow-[0_0_50px_rgba(0,0,0,0.5)] backdrop-blur-md">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center mb-10 z-10 gap-8">
                      ...
                      {assets?.slice(0, 15).map((asset) => (
                        <tr key={asset.id} onClick={() => setSelectedAsset(asset.id)} className={`border-b border-white/[0.02] last:border-0 hover:bg-violet-600/10 cursor-pointer transition-all ${selectedAssetId === asset.id ? 'bg-white/10' : ''}`}>
                          <td className="p-6">
                            ...
                            <div className="text-[11px] text-slate-100 font-normal truncate max-w-[120px]">{asset.name}</div>
                          </td>
...
function SidebarItem({ icon, label, active = false, expanded = false }: { icon: React.ReactNode; label: string; active?: boolean; expanded?: boolean }) {
  return (
    <div className={`flex items-center gap-6 p-5 rounded-2xl cursor-pointer transition-all ${active ? 'bg-white text-black shadow-[0_0_30px_rgba(255,255,255,0.2)]' : 'text-white hover:bg-white/5'}`}>
