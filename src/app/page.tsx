...
export default function Dashboard() {
  ...
  if (!assets || assets.length === 0) return (
    <div className="flex items-center justify-center h-screen bg-[#0a0518]">
      <div className="flex flex-col items-center gap-6 p-12 bg-white/5 border border-white/10 rounded-[2rem] max-w-lg text-center backdrop-blur-xl">
        ...
        <button onClick={() => { window.location.reload(); }} className="mt-6 px-8 py-3 bg-white text-black rounded-xl font-bold text-xs uppercase tracking-widest hover:scale-105 transition-transform">{t.reset_conn}</button>
      </div>
    </div>
  );
      ...
      >
        <div className="flex items-center gap-3 mb-16 px-2 overflow-hidden text-white">
          ...
          <AnimatePresence mode="wait">
            {isSidebarExpanded && (
              <motion.span initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="font-bold text-xl tracking-tighter uppercase whitespace-nowrap">EXUS</motion.span>
            )}
          </AnimatePresence>
        </div>
        ...
        <div className="p-8 lg:p-10 max-w-[1700px] mx-auto pb-32">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-12 text-white">
            ...
            <div className="xl:col-span-4 flex flex-col gap-10">
              <div>
                <h3 className="text-violet-300 text-xs font-bold tracking-widest uppercase mb-8 flex items-center gap-2">
...
function StatRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between items-center py-4 border-b border-white/5 last:border-0">
      <span className="text-white text-[10px] font-medium uppercase tracking-widest">{label}</span>
